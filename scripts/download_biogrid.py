"""Download the BioGRID S. cerevisiae S288c interaction dataset
(organism mitab archive).
...
TARGET_MEMBER = (
    f"BIOGRID-ORGANISM-Saccharomyces_cerevisiae_S288c-"
    f"{BIOGRID_VERSION}.mitab.txt"
)
    ...
    with zipfile.ZipFile(io.BytesIO(response.content)) as zf:
        if TARGET_MEMBER not in zf.namelist():
            raise FileNotFoundError(
                f"{TARGET_MEMBER} not found in archive. "
